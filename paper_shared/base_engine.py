...
class CompEngine:
    ...
    def get_agent_stats(self, name):
        ...
        "win_rate_long":  round(len([t for t in wins   if t.get("side","LONG")=="LONG"]) / max(1,len([t for t in closed if t.get("side","LONG")=="LONG"]))*100,1),
        "win_rate_short": round(len([t for t in wins   if t.get("side","LONG")=="SHORT"])/ max(1,len([t for t in closed if t.get("side","LONG")=="SHORT"]))*100,1),
        "total_long":  len([t for t in closed if t.get("side","LONG")=="LONG"]),
        "total_short": len([t for t in closed if t.get("side","LONG")=="SHORT"]),
