"""
Jan 1 - June 24 2026 Backtest — All 5 agents, LONG+SHORT, x50 leverage
$10,000 capital | $500 margin | 10 pairs
"""
...
# Jan 1 2026 00:00:00 UTC  ->  June 24 2026 23:59:59 UTC
START_MS = 1767225600000
END_MS   = 1782345599000
...
def fetch_range(pair, tf):
    """Fetch candles for exact date range, paginating if needed."""
    all_c = []
    start = START_MS
    # add 200 extra candles before start for indicator warmup
    warmup = {"15m": 200*15*60*1000, "1h": 200*3600*1000}.get(tf, 200*3600*1000)
    fetch_start = START_MS - warmup

    cur_start = fetch_start
...
def backtest_agent(agent_name, cfg):
    ...
    for pair in PAIRS:
        ...
        # find index where actual backtest period starts
        start_i = 60
...
print(f"\n{'='*68}", flush=True)
print(f"  JAN 1 - JUNE 24 2026 Backtest - x50 Leverage", flush=True)
...
for agent_name, cfg in AGENTS.items():
    ...
    print(f"  → {len(trades)} trades (L:{len(longs)} S:{len(shorts)}) | WR={wr}% | PnL=${total_pnl:+,.0f} (L:${long_pnl:+,.0f} S:${short_pnl:+,.0f})\n", flush=True)
    results.append((agent_name, cfg, trades, wr, long_pnl, short_pnl, total_pnl))

print(f"\n{'='*68}", flush=True)
print(f"  JAN 1 - JUNE 24 2026 SUMMARY", flush=True)
print(f"{'='*68}", flush=True)
print(f"  {'Agent':<18} {'TF':<5} {'SL':>5} {'TP':>5} {'Trades':>7} {'WR%':>7} {'Long PnL':>12} {'Short PnL':>12} {'Total PnL':>12}", flush=True)
print(f"  {'-'*88}", flush=True)
results.sort(key=lambda x: -x[6])
for agent_name, cfg, trades, wr, long_pnl, short_pnl, total_pnl in results:
    emoji = cfg['emoji']
    print(f"  {emoji} {agent_name:<16} {cfg['timeframe']:<5} {cfg['sl']*100:>4.1f}% {cfg['tp']*100:>4.1f}% {len(trades):>7} {wr:>7}% ${long_pnl:>+10,.0f} ${short_pnl:>+10,.0f} ${total_pnl:>+10,.0f}", flush=True)
print(flush=True)
